...
# Install the Basecamp CLI (a static Go binary). The installer drops it into
# root's ~/.local/bin, which is unreadable by the non-root app user, so relocate
# it to /usr/local/bin (world-executable and already on PATH).
RUN apt-get update \
